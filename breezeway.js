...
async function syncTasksForDate(date) {
  ...
  const insertMany = db.transaction(() => {
    for (const t of allTasks) {
      ...
      // NEVER downgrade status from Finished/Closed/Completed - once a job is done, it stays done
      const existingStatus = (existing?.bw_status || "").toLowerCase();
      const isExistingDone = ['finished','closed','completed'].includes(existingStatus);
      let status;
      if (rawStatus) {
        status = rawStatus; // Use new status from Breezeway if available
      } else if (isExistingDone) {
        status = existing.bw_status; // Preserve finished status even if BW returns empty
      } else {
        status = existing?.bw_status || ""; // Preserve any existing status
      }
      ...
      const CLEANER_RATES = {
        ...
        "nbras elrebate": {
          "1 benson fuller": 270, "32 benson fuller": 270
        },
        "astrid g": {
